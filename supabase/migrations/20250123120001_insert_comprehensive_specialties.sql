-- Insert comprehensive list of medical specialties
-- This script adds all possible medical specialties with descriptions and sort order
...
('Pediatric Nephrology', 'Children''s kidney specialist', 95),
('Pediatric Urology', 'Children''s urinary system specialist', 96),
('Pediatric Orthopedics', 'Children''s bone and joint specialist', 97),
('Pediatric Plastic Surgery', 'Children''s reconstructive surgery specialist', 98),
('Pediatric Neurosurgery', 'Children''s brain surgery specialist', 99),
('Pediatric Cardiothoracic Surgery', 'Children''s heart surgery specialist', 100)
...
UPDATE specialties SET updated_at = NOW() WHERE name IN (
  SELECT name FROM specialties WHERE updated_at IS NULL
);
